...
type ChainListJson map[uint64][]string

var chainlist ChainListJson = make(map[uint64][]string)
var chainlistMutex sync.Mutex
var nextUpdateChainlistTime time.Time

func getChainList() ChainListJson {
	if nextUpdateChainlistTime.Before(time.Now()) {
		chainlistMutex.Lock()

		regx, err := regexp.Compile(`export const extraRpcs = ({[\s\S]*})`)
		if err != nil {
			panic(err)
		}

		extraRpcs := string(regx.Find([]byte(rpcsChainlistJs)))
		extraRpcs = strings.Replace(extraRpcs, "export const extraRpcs = ", "", 1)
		...
		comments := []string{}
		for _, c := range regx.FindAll([]byte(extraRpcs), -1) {
			comments = append(comments, string(c))
		}
		...
		for _, c := range comments {
			extraRpcs = strings.Replace(extraRpcs, c, "", 1)
		}

		regx, err = regexp.Compile(`,\n[\s]*trackingDetails: privacyStatement..*,`)
		if err != nil {
			panic(err)
		}
		...
		for _, c := range regx.FindAll([]byte(extraRpcs), -1) {
			trackingDetails = append(trackingDetails, string(c))
		}

		// Remove all trackingDetails
		for _, c := range trackingDetails {
			extraRpcs = strings.Replace(extraRpcs, c, "", 1)
		}
		...
		for _, c := range regx.FindAll([]byte(extraRpcs), -1) {
			...
			chainIds = append(chainIds, chainId[:len(chainId)-2])
		}

		// fmt.Println("chainIds: ", chainIds)

		allRpcsForChains := [][]string{}
		regx, err = regexp.Compile(`rpcs: \[[\s\S]*?\]`)
		if err != nil {
			panic(err)
		}
		...
		for _, c := range regx.FindAll([]byte(extraRpcs), -1) {
			...
			toRemove := []string{}
			for _, c := range regx.FindAll([]byte(rpcs), -1) {
				remove := string(c)
				toRemove = append(toRemove, remove[:len(remove)-1])
			}

			for _, c := range toRemove {
				rpcs = strings.Replace(rpcs, c, "", 1)
			}
			...
			for _, c := range regx.FindAll([]byte(rpcs), -1) {
				rpcs := string(c)
				rpcs = strings.ReplaceAll(rpcs, "\"", "")
				allRpcs = append(allRpcs, rpcs)
			}

			allRpcsForChains = append(allRpcsForChains, allRpcs)
		}

		// fmt.Printf("chainIds: %v, allRpcs: %v\n", len(chainIds), len(allRpcsForChains))

		for i, chainId := range chainIds {
			rpcs := allRpcsForChains[i]
			// fmt.Printf("%v => %v\n", chainId, rpcs)

			chainIdBig, _ := big.NewInt(0).SetString(chainId, 10)
			chainlist[chainIdBig.Uint64()] = rpcs
			// js := gjson.Parse(rpcs)

			// fmt.Printf("%v => %v(%v)\n", chainId, js, gjson.Valid(rpcs))
		...
		chainlistMutex.Unlock()
		nextUpdateChainlistTime = time.Now().Add(1 * time.Minute)
	}

	return chainlist
}

const rpcsChainlistJs string = `export const extraRpcs = {
	1: {
	  rpcs: [
		// Quicknode -> tracks IP
		{
